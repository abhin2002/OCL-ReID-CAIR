...
from mmtrack.models.reid.losses import init_part_based_triplet_loss, CrossEntropyLoss
# from mmtrack.models.torchreid.utils.constants import GLOBAL, FOREGROUND, CONCAT_PARTS, PARTS
...
class GiLtLoss(nn.Module):
    ...
    def __init__(self,
        ...
        self.pred_accuracy = Accuracy(task="multiclass", num_classes=2, top_k=1)
        if use_gpu:
        ...
        self.identity_loss = CrossEntropyLoss(eps=ce_smooth, label_smooth=True)
        # self.identity_loss = CrossEntropyLoss(label_smooth=False)
    ...
    def forward(self, embeddings_dict, visibility_scores_dict, id_cls_scores_dict, pids):
        ...
        # for key in [GLOBAL, CONCAT_PARTS, PARTS]:
        ...
        # for key in [GLOBAL, CONCAT_PARTS, PARTS]:
            ...
            if tr_w > 0:
                parts_triplet_loss, parts_triplet_losses, parts_trivial_triplets_ratio, parts_valid_triplets_ratio = \
                ...
                loss_info['ts'] = parts_triplet_losses
    ...
    def compute_triplet_loss(self, embeddings, visibility_scores, pids):
        ...
        losses = self.part_triplet_loss(embeddings, pids, parts_visibility=visibility)
        parts_triplet_losses = losses[0]
        triplet_loss, origin_loss, trivial_triplets_ratio, valid_triplets_ratio = losses[1]
        return triplet_loss, parts_triplet_losses, trivial_triplets_ratio, valid_triplets_ratio

    def compute_id_cls_loss(self, id_cls_scores, visibility_scores, pids):
        if len(id_cls_scores.shape) == 3:
            M = id_cls_scores.shape[1]  # 2 or 1
            id_cls_scores = id_cls_scores.flatten(0, 1)  # (B, 2)
        ...
        # print("id_cls_scores:", id_cls_scores)
        # print("pids:", pids)
        # print("weights:", weights)

        cls_loss = self.identity_loss(id_cls_scores, pids, weights)
        if self.use_visibility_scores and visibility_scores.dtype is not torch.bool:
            accuracy = self.pred_accuracy(id_cls_scores[weights==1], pids[weights==1])
        else:
