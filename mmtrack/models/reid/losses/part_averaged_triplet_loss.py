...
class PartAveragedTripletLoss(nn.Module):
    ...
    def forward(self, part_based_embeddings, labels, parts_visibility=None):
        ...
        part_based_pairwise_dist = self._part_based_pairwise_distance_matrix(part_based_embeddings.transpose(1, 0), squared=False)  # (K,N,N)
        ...
        part_dist, pairwise_dist = self._combine_part_based_dist_matrices(part_based_pairwise_dist, valid_part_based_pairwise_dist_mask, labels)  # (K,N,N), (N,N)

        part_losses = self._part_hard_mine_triplet_loss(part_dist, labels, self.margin)  # (K)
        # mean_part_loss = self._hard_mine_triplet_loss(part_dist, labels, self.margin)  # (1)
        mean_part_loss = self._hard_mine_triplet_loss(pairwise_dist, labels, self.margin)

        # if mean_part_loss[0].item() == 0.0:
        #     print()

        return part_losses, mean_part_loss
    ...
    def _combine_part_based_dist_matrices(self, part_based_pairwise_dist, valid_part_based_pairwise_dist_mask, labels):
        if valid_part_based_pairwise_dist_mask is not None:
            ...
            part_dist, pairwise_dist = masked_mean(part_based_pairwise_dist, valid_part_based_pairwise_dist_mask)
        else:
            valid_part_based_pairwise_dist = part_based_pairwise_dist
            pairwise_dist = valid_part_based_pairwise_dist.mean(0)

        return part_dist, pairwise_dist
    ...
    def _part_hard_mine_triplet_loss(self, batch_pairwise_dist, labels, margin):
        ...
        # DO NOT use negatives samples in triplet loss (Hanjing)
        hardest_positive_dist = hardest_positive_dist[:, labels==1]  # [K, N//2]
        ...
        # DO NOT use negatives samples in triplet loss (Hanjing)
        hardest_negative_dist = hardest_negative_dist[:, labels==1]  # [K, N//2]
        ...
        valid_triplets_mask = valid_hardest_positive_dist_mask * valid_hardest_negative_dist_mask  # [K, N//2]
        hardest_dist = torch.stack([hardest_positive_dist, hardest_negative_dist], 2)  # [K, N//2, 2]
        valid_hardest_dist = hardest_dist[valid_triplets_mask, :]  # [K*N//2, 2]
        ...
        if self.margin > 0:
            part_triplet_losses = F.relu(hardest_dist[:, :, 0] - hardest_dist[:, :, 1] + margin)  # [K, N//2]
            part_triplet_losses = torch.mul(part_triplet_losses, valid_triplets_mask)  # [K, N//2]
            part_triplet_losses = torch.sum(part_triplet_losses, dim=1)  # [K]
            part_masks_sum = torch.sum(valid_triplets_mask, dim=1)  # [K]
            part_masks_sum_clamp = torch.clamp(part_masks_sum, min=1e-6)  # [K]
            triplet_losses_avg = torch.div(part_triplet_losses, part_masks_sum_clamp)
            triplet_losses_avg[part_masks_sum==0] = -1

        else:
            warnings.warn("No valid margin")
            return None

        return triplet_losses_avg
    ...
    def _hard_mine_triplet_loss(self, batch_pairwise_dist, labels, margin):
        ...
        # Build valid triplets and compute triplet loss
        if self.margin > 0:
            triplet_loss, origin_dist, trivial_triplets_ratio, valid_triplets_ratio = self.hard_margin_triplet_loss(margin, valid_hardest_dist,
                                                                                           valid_triplets_mask)
        else:
            triplet_loss, trivial_triplets_ratio, valid_triplets_ratio = self.soft_margin_triplet_loss(0.3, valid_hardest_dist,
                                                                                           valid_triplets_mask)

        return triplet_loss, origin_dist, trivial_triplets_ratio, valid_triplets_ratio
    ...
    def hard_margin_triplet_loss(self, margin, valid_hardest_dist, valid_triplets_mask):
        ...
        origin_dist = valid_hardest_dist[:, 0] - valid_hardest_dist[:, 1]
...
def masked_mean(input, mask):
    ...
    ### Hanjing ###
    valid_part_dist =replace_values(valid_input, ~mask, -1)
    ### Hanjing ###
    ...
    return valid_part_dist, valid_pairwise_dist
