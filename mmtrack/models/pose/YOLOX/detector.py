...
class PersonDetector(Detector):
    def __init__(self, model='yolox-s', ckpt='./weights/yolox_s.pth.tar'):
        Detector.__init__(self, model=model, ckpt=ckpt)

        self.filter_class = "person"
    
    def detect(self, img, img_metas, conf=0.5, rescale=False):
        """
        img: Scaled Tensor
        img_metas: 
        """
        ...
        with torch.no_grad():
            ...
            if (outputs!=None):
                ...
                if rescale:
                    info['boxes'] = outputs[:, 0:4] / img_metas[0]['scale_factor']
                info['boxes'][0::2] = np.clip(info['boxes'][0::2], a_min=0, a_max=ori_w)
                info['boxes'][1::2] = np.clip(info['boxes'][1::2], a_min=0, a_max=ori_h)
                ...
                for xyxy, class_id, score  in zip(info['boxes'],info['class_ids'],info['scores']):
                    if class_names[int(class_id)] in self.filter_class and score > conf:
                        ...
                        xyxy_score = np.zeros(5)
                        xyxy_score[:4] = xyxy
                        xyxy_score[4] = score
                        xyxys.append(xyxy_score)
                        # scores.append(score)
        return xyxys
