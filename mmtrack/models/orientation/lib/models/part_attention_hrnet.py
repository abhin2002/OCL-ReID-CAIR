...
class PartAttentionHRNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.conv_local_feature_layer1 = self._make_conv_layer(2, (96, 96), (3, 3))
        ...
        self.part_attention = PartAttention(
            act='softmax')
        self.conv_local_feature_layer2 = self._make_conv_layer(2, (128, 128), (3, 3))
        # self.inplanes = 17
        # self.hoe_layer1 = self._make_layer(BasicBlock, 64, 2)
        self.hoe_layer2 = self._make_layer1d(BasicBlock1D, 128, 2, stride=2)
        ...
        self.mask_weight_layer1 = nn.Conv2d(
                in_channels=self.mask_channel-1,
                out_channels=12,
                kernel_size=2,
                stride=2,
                padding=0,
        ...
        self.mask_select_fc = nn.Linear(24, self.mask_channel - 1)
    ...
    def _make_layer1d(self, block, planes, blocks, stride=1):
        downsample = None
        channel_num = int(self.mask_channel-1)
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                LocallyConnected2d(
                    self.inplanes, planes * block.expansion,
    ...
    def forward(self, x):
        ...
        attention_mask_weight = self._get_mask_weight(heatmaps[:,:self.mask_channel-1,:,:])
        ...
        embedding = self.hoe_layer2(weighted_attention_feature)
