...
class BasicBlock1D(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, feature_num=None):
        super(BasicBlock1D, self).__init__()
        self.conv1 = LocallyConnected2d(inplanes, planes[0], output_size=[int(feature_num),1], kernel_size = 1, stride=1)
        self.bn1 = nn.BatchNorm1d(planes[0], momentum=BN_MOMENTUM)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = LocallyConnected2d(planes[0], planes[1], output_size=[int(feature_num),1], kernel_size = 1, stride=1)
        self.bn2 = nn.BatchNorm1d(planes[1], momentum=BN_MOMENTUM)
...
class PartAttentionHRNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        self.cfg = cfg
        self.img_width = cfg.MODEL.IMAGE_SIZE[0]
        self.img_height = cfg.MODEL.IMAGE_SIZE[1]
        self.aspect_ratio = self.img_width * 1.0 / self.img_height
        self.pixel_std = 200
        ...
        self.hoe_layer2 = self._make_layer1d(BasicBlock1D, planes=[2,1], blocks=1, stride=1)
    ...
    def _make_layer1d(self, block, planes, blocks, stride=1):
        ...
        self.inplanes = self.mask_channel - 1
        feature_num = int(128)
        if stride != 1 or self.inplanes != planes[-1] * block.expansion:
            downsample = nn.Sequential(
                LocallyConnected2d(
                    self.inplanes, planes[-1] * block.expansion,
                    output_size=[feature_num, 1], kernel_size = 1, stride=1
                ),
                nn.BatchNorm1d(planes[-1] * block.expansion, momentum=BN_MOMENTUM),
            )
        logger.info('_make_layer1d {} {} {}'.format(self.inplanes, planes, block.expansion))

        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, feature_num=feature_num))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, feature_num=feature_num))
    ...
    def forward(self, x):
        ...
        attention_feature = self.part_attention(feature_2d, heatmaps[:,1:,:,:])
        ...
        return x, y, heatmaps[:, 1:, :, :]  # only head torso leags feet
    ...
    def predict(self, image, img_metas, bboxs, bboxs_scores, bbox_ids=None, rescale=False):
        ...
        pre_key_heatmaps, orientations, pre_parsings = self.forward(inps.cuda())
        ...
        parsings = np.zeros(pre_parsings.shape)  # B,5,64,48
        ...
        for i in range(pre_parsings.shape[0]):
            ...
            transform_parse = pre_parsings[i].transpose(1,2,0)
    ...
    def _crop_dets(self, img, boxes, height, width):
        """
        image: torch tensor (C,H,W) with BRG channel
        """
        # self.aspect_ratio = 
        img = torch_to_im(img)
        ...
        c_ses = np.zeros((len(boxes), 4))
        inverse_transes = np.zeros((len(boxes), 2, 3))
        bbxoes = np.zeros((len(boxes), 4))
        # pt2 = torch.zeros(len(boxes), 2)
        for i, box in enumerate(boxes):
            xywh = (box[0], box[1], box[2]-box[0], box[3]-box[1])
            bbxoes[i] = np.array(xywh)
            center, scale = self._box2cs(xywh)
            c_ses[i, :2] = center
            c_ses[i, 2:4] = scale
            trans = get_affine_transform(center, scale, 0, (width, height))
            inverse_transes[i] = get_affine_transform((scale*self.pixel_std/2.0).astype(np.uint16), scale, 0, [48, 64], inv=1)  # for parsing mask
            input = cv2.warpAffine(
                img,
                trans,
                (int(self.img_width), int(self.img_height)),
                flags=cv2.INTER_LINEAR)
            inps[i] = normalize(input)  # normalize
        return inps, bbxoes.astype(np.uint16), c_ses, inverse_transes
