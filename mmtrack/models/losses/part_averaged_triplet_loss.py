...
class PartAveragedTripletLoss(nn.Module):
    ...
    def __init__(self, margin=0.3, loss_weight=1.0, epsilon=1e-16, writer=None):
    ...
    def forward(self, part_based_embeddings, labels, parts_visibility=None):
        ...
        Args:
            ...
            parts_visibility (torch.LongTensor): part visibility masks with shape (4,H,W).
        """

        # Compute pairwise distance matrix for each part with (part_nums,B,B)
        ...
        return self.loss_weight * self._hard_mine_triplet_loss(pairwise_dist, labels, self.margin)

    def _combine_part_based_dist_matrices(self, part_based_pairwise_dist, valid_part_based_pairwise_dist_mask, labels):
        """
        part_based_pairwise_dist (part_nums,B,B)
        """
