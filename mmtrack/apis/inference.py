...
def init_model(config,
    ...
    """
    init_work_seed(seed)  # Very important for reproducing results
    ...
    if 'reid' in config.model:
        config.model.reid.pretrained = None
    if 'hyper_config' in config.model:
        config.model.hyper_config = hyper_config
    if 'identifier' in config.model:
        config.model.identifier = identifier_config.identifier_params
...
def inference_rpf(model, img, frame_id, gt_bbox):
    """Inference image(s) with the rpf model.

    Args:
        model (nn.Module): The loaded rpf model.
        img (str | ndarray): Either image name or loaded image.
        frame_id (int): frame id.
        gt_bbox (list): target person's gt box
    ...
    # build the data pipeline
    ### this pipeline will rescale the image ###
    test_pipeline = Compose(cfg.data.test.pipeline)
    data = test_pipeline(data)
    data = collate([data], samples_per_gpu=1)
    if next(model.parameters()).is_cuda:
    ...
    # print(data.keys())
    with torch.no_grad():
        result = model(return_loss=False, rescale=True, gt_bbox=np.array(gt_bbox), **data)
    return result

def inference_rpf_wo_gt(model, img, track_ids, track_bboxes, track_kpts, target_id, frame_id):
    """Inference image(s) with the rpf model.
    ...
    data['track_bboxes'] = track_bboxes
    data['track_kpts'] = track_kpts
    data['target_id'] = target_id
    # print(data.keys())

    with torch.no_grad():
        result = model(return_loss=False, rescale=True, gt_bbox=None, **data)
    return result

def extract_feature(model, img, frame_id, bboxes):
    """Inference image(s) with the rpf model.
    ...
    for bbox in bboxes:
        ...
        # print(data["img"])
        tracklet = Tracklet(data["img"][0], data["img_metas"][0], bbox, rescale=True)
        crop_img = tracklet.image_patch
        with torch.no_grad():
            feature = model.reid.extract_features(crop_img)
